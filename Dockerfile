...
# Health check (MCP server returns 404 on /, which still means it's up)
HEALTHCHECK --interval=30s --timeout=10s --retries=3 \
    CMD curl -sf http://localhost:8765/ || curl -sf http://localhost:8765/mcp -H 'Content-Type: application/json' -o /dev/null -w '%{http_code}' | grep -qE '^[234]' || exit 1

# Run the MCP server over streamable-http (ideal for Docker/remote deployments)
ENTRYPOINT ["tradingview-mcp"]
CMD ["streamable-http", "--host", "0.0.0.0", "--port", "8765"]
